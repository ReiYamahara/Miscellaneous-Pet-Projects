...
def worley_noise_animated(pixels = 400, num_feature_points = 16, k = 3, seed = 3, iterations = 80, 
    ...
    '''
    parameters:
    pixels: number of pixels per axis
    num_feature_points: the number of feature points
    k: k represents how much the feature point moves
    seed: random seed
    ...
    # where we store each final z plot
    ims = []
    ...
    for i in range(iterations):
        ...
        im = ax.imshow(z, animated = True, cmap = cmap, interpolation = None)
        ims.append([im])
        # new z each time
        z = np.ones((pixels, pixels))
        ...
        random_vectors = (random_vectors_possibilities[a] / pixels) * k
        ...
        feature_points[feature_points > 1] = 1
